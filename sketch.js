...
function drawChart() {
  ...
  d3.csv("ABI_Einsatzbetriebe_und_Einsatzplaetze_nach_TB_2025.csv").then(raw => {
    ...
    function showTooltip(event, d, metricLabel, color) {
      tooltip.raise();
      ...
      tooltipText.append("tspan")
        ...
        .style("font-size", "13.5px")
        .style("fill", color)
        .text(`${metricLabel} : ${formatSwiss(d.data.value)}`);
    ...
    PANELS.forEach((p, pi) => {
      ...
      cell
        .on("mouseover", function (event, d) {
          ...
          showTooltip(event, d, p.shortMetric, p.base);
        })
        .on("mousemove", (event, d) => showTooltip(event, d, p.shortMetric, p.base))
