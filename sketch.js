...
function drawChart() {
  ...
  d3.csv("ABI_Einsatzbetriebe_und_Einsatzplaetze_nach_TB_2025.csv").then(raw => {
    ...
    // ⭐ Titre sur 3 lignes égales (une par langue, même graphie) : budget
    // de hauteur plus grand que la version à 2 lignes.
    const titleHeight = isMobile ? 48 : 56;
    const panelHeight = isMobile ? 155 : 278;
    ...
    function showTooltip(event, d, metricParts, color) {
      ...
      // ⭐ Les 3 langues du nom de la métrique (pas seulement l'allemand),
      // même graphie entre elles — puis la valeur, une seule fois.
      metricParts.forEach((part, i) => {
        tooltipText.append("tspan")
          .attr("x", padX)
          .attr("dy", i === 0 ? "1.5em" : "1.15em")
    ...
    PANELS.forEach((p, pi) => {
      ...
      // Titre du panneau : puce + libellé trilingue sur 2 lignes
      ...
      // ⭐ Les 3 langues ont exactement la même graphie (taille, graisse,
      // couleur) — aucune n'est mise en avant par rapport aux autres.
      titleParts.forEach((part, i) => {
        titleText.append("tspan")
          .attr("x", 20)
          .attr("dy", i === 0 ? "0.9em" : "1.3em")
